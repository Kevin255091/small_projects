...
def main():
    ...
    filesize_smaller_than_16M = 0
    filename_too_long = []
    for f in input_file_list:
        fnb_len = len(os.path.basename(f).encode('utf-8'))
        if fnb_len > 127:
            filename_too_long.append(f)
        if os.path.getsize(f) < 16777216: # 2 ** 24
            filesize_smaller_than_16M += 1
    ...
    space_saved = len(input_file_list) + filesize_smaller_than_16M -2 - len(output_filename.encode('utf-8'))

    if space_saved <= 0:
        print(f'Saved space estimated: {space_saved} bytes.')
        r  = input('It does not save any space. Sure to proceed ? (N/y) ==> ').lower()
        if r == 'n' or r == 'no':
            print('Program terminated.')
            sys.exit()
        else:
            print('Program proceeded.')
