...
def search_filename_btn_click():
    ...
    name_len_threshold = 20
    for line in buf:
        try:
            name_len_threshold = int(line.strip().rstrip('\r\n'))
            break
        except:
            continue

    text_widget.insert(INSERT, '\n==========================================================\n')
    text_widget.insert(INSERT, '檔名超過' + str(name_len_threshold) + '個字的檔案：\n')
    ...
    for rt, dirs, files in os.walk(working_directory, topdown=True):
        ...
        for name in files:
            if name[0] != '.' and '.' in name:
                ...
                name_len = len(name)
            if name_len > name_len_threshold:
                match_count += 1
                complete_name = os.path.abspath(os.path.join(rt, name))
                text_widget.insert(INSERT, strip_outrange_char(complete_name) + '\n')
...
if __name__ == '__main__':
    ...
    instruct_info = '請在下面的 text area 輸入檔名長度門檻值，並按下 search filename 按鈕'
    ...
    search_filename_btn = Button(toolbar, text='search filenname', font=(None, font_size), command=search_filename_btn_click)
